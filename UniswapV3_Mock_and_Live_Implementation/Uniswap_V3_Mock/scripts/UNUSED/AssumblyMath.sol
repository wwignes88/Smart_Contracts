...
// ===================================================
            ...
            function MulDiv0(
                ...
                uint256 b,
                uint256 denominator
             ) public pure returns (uint256 result, uint256 prod0) {
                assembly {
                    prod0  := mul(a, b)
                    result := div(prod0, denominator)
                }
            ...
            function MOD(
                uint256 a,
                uint256 b
            ) public pure returns (uint256 mod_) {
                assembly {
                    mod_ := mulmod(a, b, not(0))
                }
            }

            // uint256 intermediate = MulDiv(sqrtRatioAX96, sqrtRatioBX96, Q96);
            // see FullMath.sol library in v3-core
            function MulDivTroubleshoot(
                uint256 a,
                uint256 b,
                uint256 denominator,
                uint8 option
        ...
        function LTEcomparator(
            uint32 time,
            uint32 a,
            uint32 b
        ) external pure returns (uint256 aAdjusted, uint256 bAdjusted, bool comparison ) {
            ...
            aAdjusted = a > time ? a : a + 2**32;
            bAdjusted = b > time ? b : b + 2**32;
            comparison = aAdjusted <= bAdjusted;
        }
